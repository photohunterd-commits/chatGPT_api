...
5. Откройте `Settings` уже внутри приложения и сохраните свой personal API key от провайдера модели.
...
То есть пользователю не нужно вручную прописывать адрес сервера или вообще видеть его в базовом сценарии.
...
Дополнительно в desktop-приложении теперь есть:

- смена пароля для авторизованного пользователя
- восстановление пароля по email, если на сервере настроен SMTP
...
- `SMTP_*` - опциональная настройка отправки welcome/reset писем
- `PASSWORD_RESET_TOKEN_TTL_MINUTES` - срок жизни токена восстановления пароля
...
- смена пароля пользователя работает
